...
class WorkerThread(QtCore.QThread):
    ...
    def calibrate(self):
        # Initialize lists for 1s and 15s calibration data
        Xf, Xe, Xr, y1 = [], [], [], []
        X15, y15 = [], []
        ...
        for label, emo in enumerate(EMOTIONS):
            ...
            # Record camera while video plays
            ...
            cap_v.release()
            out_cam.release()
            # Stop video playback briefly to prepare next
            self.video_to_play.emit("")
            time.sleep(0.2)
            self.progress.emit(f"[Calibration] Saved camera recording for {emo} at {out_path}")
            # Load recorded camera video frames
            cap_rec = cv2.VideoCapture(out_path)
            recorded = []
            while True:
            ...
            cap_rec.release()
            total = len(recorded)
            # 1s segments: non-overlapping
            seg_len1 = int(SEG_SEC_1 * FPS)
            for start in range(0, total - seg_len1 + 1, seg_len1):
                seg = recorded[start:start+seg_len1]
                ...
                if f_feats.size and e_feats.size and r_feats.size:
                    ...
                    self.progress.emit(f"[Calibration] Skipping 1s segment at {start}/{total} for {emo}")
            # 15s segments: non-overlapping
            seg_len15 = int(SEG_SEC_15 * FPS)
            for start in range(0, total - seg_len15 + 1, seg_len15):
                clip = recorded[start:start+seg_len15]
                arr = np.stack(clip)
                comb, valid = compute_combined_scores_15(arr, np.ones(len(arr), bool), np.zeros((len(arr),N_LM,2)))
                if comb.size == 0:
                    self.progress.emit(f"[Calibration] Skipping 15s segment at {start}/{total} for {emo}: no face detected")
                    continue
