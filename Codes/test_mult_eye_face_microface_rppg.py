...
CALIB_1_PATH     = 'calib_data_1_input3.npz'
# 15s model
SEG_SEC_15       = 15
FPS              = 30
...
def safe_extract_1s_features(frames, fps, me, se, mf, sf, mr, sr):
    ...
    if f_feats.size == 0 or e_feats.size == 0 or r_feats.size == 0:
        return None
    ...
    try:
        ...
        return None
    return [inp_eye, inp_face, inp_rppg]
...
class WorkerThread(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            res = f"Error: {e}"
        if self.mode != 'calibrate':
            self.video_to_play.emit("")  # evaluate 끝날 때만 영상 정지
    ...
    def calibrate(self):
        ...
        for label, emo in enumerate(EMOTIONS):
            ...
            for start in range(0, total - seg1 + 1, seg1):
                ...
                if f_feats.size and e_feats.size and r_feats.size:
                    Xf_new.append(f_feats[:,0]); Xe_new.append(e_feats[:,0]); Xr_new.append(r_feats.T); y1_new.append(label)
                else:
        ...
        if Xf_old is not None:
            Xf = np.concatenate([Xf_old, np.stack(Xf_new)], axis=0)
            Xe = np.concatenate([Xe_old, np.stack(Xe_new)], axis=0)
            Xr = np.concatenate([Xr_old, np.stack(Xr_new)], axis=0)
            y1 = np.concatenate([y1_old, np.array(y1_new)], axis=0)
        else:
            Xf = np.stack(Xf_new) if Xf_new else np.empty((0,0))
            Xe = np.stack(Xe_new) if Xe_new else np.empty((0,0))
            Xr = np.stack(Xr_new) if Xr_new else np.empty((0,0,0))
            y1 = np.array(y1_new) if y1_new else np.array([])
        if X15_old is not None:
            X15 = np.concatenate([X15_old, np.stack(X15_new)], axis=0)
            y15 = np.concatenate([y15_old, np.array(y15_new)], axis=0)
        else:
            X15 = np.stack(X15_new) if X15_new else np.empty((0,ONOFF_LEN*3,64,64,1))
            y15 = np.array(y15_new) if y15_new else np.array([])
        ...
        model1.fit([
            ((Xe-me)/se)[:,None,:], ((Xf-mf)/sf)[:,None,:], ((Xr-mr)/sr)
    ...
    def evaluate(self):
        ...
        for i in range(15):
            ...
            if len(buf1) < buf1.maxlen:
                ...
                continue
            inputs = safe_extract_1s_features(list(buf1), fps, me, se, mf, sf, mr, sr)
            if inputs is None:
